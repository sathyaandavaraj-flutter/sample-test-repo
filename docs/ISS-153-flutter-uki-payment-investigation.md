# ISS-153 — Flutter UKI payment/withdrawal investigation & triage runbook
...
> **DevRev:** `ISS-153` (`don:core:dvrv-us-1:devo/1pyquBtpWW:issue/153`)

## ⚠️ This is a duplicate

ISS-153 was auto-created from a Slack thread and carries the **same
Slack-sourced template** as a cluster of earlier issues: **ISS-148, ISS-149,
ISS-150, ISS-151, and ISS-152**. The on-call triage notes describe the same
**payment / withdrawal failure of unknown cause** (payment gateway, internal
...
already **In Development**. ISS-153 has therefore been linked as a duplicate of
ISS-148 in DevRev and moved to the `duplicate` stage so a single investigation
tracks the work instead of six parallel ones.

This document is retained as the per-issue artifact for ISS-153 and mirrors the
...
Because ISS-153 is a duplicate, it is **done when it is consolidated**: linked as a
...
| **ISS-153** | Duplicate (this issue) — linked to ISS-148 |
