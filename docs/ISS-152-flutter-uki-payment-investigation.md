# ISS-152 — Flutter UKI payment/withdrawal investigation & triage runbook
...
> **DevRev:** `ISS-152` (`don:core:dvrv-us-1:devo/1pyquBtpWW:issue/152`)

## ⚠️ This is a duplicate

ISS-152 was auto-created from a Slack thread and carries the **same title and
Slack-sourced template** as a cluster of earlier issues: **ISS-148, ISS-149,
ISS-150, and ISS-151**. The on-call triage notes describe the same
**payment / withdrawal failure of unknown cause**.

The earliest issue in the cluster, **ISS-148**, is the canonical record and is
already **In Development**. ISS-152 has therefore been linked as a duplicate of
ISS-148 in DevRev and moved to the `duplicate` stage so a single investigation
tracks the work instead of five parallel ones.

This document is retained as the per-issue artifact for ISS-152 and mirrors the
...
Because ISS-152 is a duplicate, it is **done when it is consolidated**: linked as a
...
| **ISS-152** | Duplicate (this issue) — linked to ISS-148 |
