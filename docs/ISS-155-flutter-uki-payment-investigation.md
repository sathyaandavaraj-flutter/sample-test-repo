# ISS-155 — Flutter UKI payment/withdrawal investigation & triage runbook
...
> **DevRev:** `ISS-155` (`don:core:dvrv-us-1:devo/1pyquBtpWW:issue/155`)

## ⚠️ This is a duplicate

ISS-155 was auto-created from a Slack thread and carries the **same
Slack-sourced template** as a cluster of earlier issues: **ISS-148, ISS-149,
ISS-150, ISS-151, ISS-152, ISS-153, and ISS-154**. The on-call triage notes
describe the same **payment / withdrawal failure of unknown cause** (payment
gateway, internal wallet, bank/settlement, system error, or product
configuration).

The earliest issue in the cluster, **ISS-148**, is the canonical record and is
already **In Development**. ISS-155 has therefore been linked as a duplicate of
ISS-148 in DevRev and moved to the `duplicate` stage so a single investigation
tracks the work instead of eight parallel ones.

This document is retained as the per-issue artifact for ISS-155 and mirrors the
...
Because ISS-155 is a duplicate, it is **done when it is consolidated**: linked as a
...
| ISS-154 | Duplicate of the same Slack-sourced incident — linked to ISS-148 |
| **ISS-155** | Duplicate (this issue) — linked to ISS-148 |
